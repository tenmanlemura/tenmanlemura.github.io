...
2. 作業対象のプロジェクトが「お金が動く（money_exposure: true）」かを `related-projects.json`（下記「探す場所」）で確認する
3. money_exposure: true の場合は **このセッションで編集をしない**。Claude / Codex に回すよう促す（read-only の確認・調査までは可）

## 参照ファイル（3 点セット）

- `COMMON.md` — AI 判断主義 / 出力フォーマット / 探索ゲート / 委任プロトコル / Gemini 委任ルール
- `SECURITY.md` — secrets / 個人情報 / untrusted data / 機械防壁の迂回禁止 / skill 取扱い
- `related-projects.json` — どのプロジェクトがどこにあり、お金が動くか（money_exposure）の一覧

探す場所（上から順に試し、最初に見つかった場所を使う。配布スクリプト `distribute-skills.sh` は SKILL.md しかコピーしないため、配布先リポには `references/` が無いことがある）:

1. このスキルと同じディレクトリの `references/`
2. yuco 正本: `~/Documents/yuco/core/ai-rules/COMMON.md`・`~/Documents/yuco/core/ai-rules/SECURITY.md`・`~/Documents/yuco/core/related-projects.json`

どちらにも無い（yuco 正本リポが無いマシン等）場合は、読めなかったファイル名を明示して yuko に報告し、money_exposure は **不明 = true 扱い（fail closed）** で進める。
