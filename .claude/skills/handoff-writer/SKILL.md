...
使わない場面: おまかせモードの終了時は `omakase-end` が担当（本 skill と混同しない）。通常セッションの締めは常に本 skill（COMMON.md「セッション切り替え時の handoff」準拠）。
...
1. **「未復元の副作用」は必ず書く**：state ファイル / 環境変数 / リポジトリ外ファイル（`~/.config/yuco/` 配下等）を書き換えるテストをして未復元なら、冒頭近くに明記。無ければ「該当なし（state / 環境変数 / repo 外ファイルの書き換えなしを確認済）」と書く。**空欄禁止**

   **良い例**（実績出典: work-trust-dev `docs/handoff/20260702_1744.md`「未復元の副作用」）— 副作用を 3 分類で書くと次セッションが判断に迷わない:
   - **意図的・恒久**: デプロイ・本番反映そのもの（戻さない。バックアップ等の残置物があれば掃除タスクとして明記）
   - **無害・掃除任意**: ビルド生成物・scratch ファイル（`.tmp-*` 等）
   - **git 状態**: push 済 / ahead 件数・今回触っていない `M` / `??` の帰属（「別作業のもの・commit しない」を明示）
